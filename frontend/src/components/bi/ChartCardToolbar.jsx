...
export default function ChartCardToolbar({ onExpand, isDarkMode = false, compact = false }) {
  const btn = compact
    ? 'rounded border p-0.5 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/40'
  ...
  return (
    ...
    >
      <MdOpenInFull className={compact ? 'h-3 w-3' : 'h-4 w-4'} />
    </button>
  );
}
