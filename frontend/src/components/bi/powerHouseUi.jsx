...
export function BandCard({ title, children, dm, className = '', bodyClassName = '', right = null, tone = null }) {
  return (
    <div
    ...
    >
      {title ? (
        tone ? (
          <div className={`shrink-0 px-3 py-1.5 flex items-center justify-between gap-2 rounded-t-2xl bg-gradient-to-r ${HEADER_TONES[tone] || HEADER_TONES.blue}`}>
            <h3 className="text-[10px] font-black uppercase tracking-[0.14em] text-white truncate">{title}</h3>
            {right}
          </div>
        ) : (
          <div className="shrink-0 px-4 pt-3.5 pb-1 flex items-center justify-between gap-2">
      ...
      <div className={`flex-1 min-h-0 overflow-hidden ${title && !tone ? 'px-3 pb-2.5' : 'p-2'} ${bodyClassName}`}>{children}</div>
    </div>
  );
}

/** Cane Gate-1 KPI: circular pastel icon, label, bold value, info tip */
export function KPICard({ label, value, unit = '', color = 'blue', dm, compact = false, icon: Icon, info }) {
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1 pr-5">
        <p className={`text-[11px] font-semibold leading-tight ${dm ? 'text-slate-400' : 'text-slate-500'}`}>{label}</p>
        <p className={`${compact ? 'text-lg' : 'text-2xl'} font-black tracking-tight tabular-nums mt-0.5 ${dm ? 'text-slate-50' : 'text-slate-900'}`}>
          {display}
          {unit ? (
            <span className={`ml-1 text-[10px] font-semibold ${dm ? 'text-slate-400' : 'text-slate-500'}`}>{unit}</span>
          ) : null}
        </p>
      </div>
    </div>
...
export function MetricLine({ label, value, dm, emphasize = false }) {
  return (
    <div className={`flex items-baseline justify-between gap-2 ${emphasize ? 'py-0.5' : ''}`}>
      <span className={`text-[9px] font-bold uppercase tracking-wide ${dm ? 'text-slate-400' : 'text-slate-500'}`}>
        {label}
      </span>
      <span className={`tabular-nums font-black ${emphasize ? 'text-base' : 'text-sm'} ${dm ? 'text-slate-100' : 'text-slate-900'}`}>
