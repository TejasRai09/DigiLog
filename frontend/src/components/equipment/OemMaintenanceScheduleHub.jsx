                        ...
                        <tbody className="divide-y divide-slate-100">
                          {filtered.map((row, index) => (
                            <tr key={row.id} className="hover:bg-slate-50/40 align-top">
                              ...
                              <td className="py-3 px-4 font-semibold text-slate-900 text-sm">{row.component}</td>
                              ...
                              <td className="py-3 px-4">{renderIntervalBadges(row.intervals)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                        ...
                        <div key={row.id} className="bg-white border border-slate-200 rounded-xl p-3.5 shadow-sm space-y-2.5">
                          <div className="flex items-start justify-between gap-2">
                            <div className="flex items-center gap-2 min-w-0">
                              <span className="text-[10px] font-bold bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">#{index + 1}</span>
                              <h4 className="font-bold text-slate-900 text-xs leading-tight">{row.component}</h4>
                            </div>
                            {renderIntervalBadges(row.intervals)}
                          </div>
                          <ul className="list-disc pl-4 text-[11px] text-slate-600 space-y-1 border-t border-slate-100 pt-2">
                            {row.actions.map((step, i) => (
                              <li key={i} className="break-words">{step}</li>
                            ))}
                          </ul>
                                ...
                                <div className="space-y-1 max-h-32 overflow-y-auto">
                                  {row.actions.map((step, stepIdx) => (
                                    <div key={stepIdx} className="flex items-center gap-1">
                                      ...
                                      <input type="text" value={step} onChange={(e) => updateAction(row.id, stepIdx, e.target.value)} className="flex-1 px-2 py-1 text-xs border border-slate-200 rounded" placeholder={`Step ${stepIdx + 1}`} />
                                      {row.actions.length > 1 && (
                                        <button type="button" onClick={() => deleteAction(row.id, stepIdx)} className="text-slate-300 hover:text-rose-500"><MdClose className="w-3.5 h-3.5" /></button>
                                      )}
                                    </div>
                                  ))}
                                </div>
                              ...
                              <div className="mt-1 space-y-1.5">
                                {row.actions.map((step, stepIdx) => (
                                  <div key={stepIdx} className="flex items-center gap-2">
                                    <span className="text-slate-300">•</span>
                                    <input type="text" value={step} onChange={(e) => updateAction(row.id, stepIdx, e.target.value)} className="flex-1 px-2 py-1.5 text-sm border border-slate-200 rounded-lg" />
                                    {row.actions.length > 1 && (
                                      <button type="button" onClick={() => deleteAction(row.id, stepIdx)} className="text-slate-400 hover:text-rose-600"><MdClose className="w-4 h-4" /></button>
                                    )}
                                  </div>
                                ))}
