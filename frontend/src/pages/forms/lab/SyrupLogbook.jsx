...
import { buildSyrupLogbookReview } from '../../../config/gsmaFormReviewBuilders';
...
const SyrupLogbook = () => {
  ...
  const { reviewOpen, submitting, openReview, closeReview, confirmSubmit } = useGsmaFormReview({
    ...
    submit: async () => {
      await gsmaSubmitRequest(
        () => api.post('/forms/syrp_logbook', form),
        'Syrup Logbook submitted!',
      );
      setForm(INITIAL);
    },
  });
  ...
  const reviewConfig = useMemo(
    () => (reviewOpen ? buildSyrupLogbookReview(form) : null),
    [reviewOpen, form],
  );

  return (
    <main className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-8">
      <FormPageHeader formKey="syrp_logbook" fallbackTitle="Syrup Logbook" />
