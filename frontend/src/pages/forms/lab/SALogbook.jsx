...
import { buildSALogbookReview } from '../../../config/gsmaFormReviewBuilders';
...
const SALogbook = () => {
  ...
  const { reviewOpen, submitting, openReview, closeReview, confirmSubmit } = useGsmaFormReview({
    ...
    submit: async () => {
      await gsmaSubmitRequest(
        () => api.post('/forms/sa_logbook', form),
        'Special Analysis Logbook submitted!',
      );
      setForm(INITIAL);
    },
  });
  ...
  const reviewConfig = useMemo(
    () => (reviewOpen ? buildSALogbookReview(form) : null),
    [reviewOpen, form],
  );

  return (
    <main className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-8">
      <FormPageHeader formKey="sa_logbook" fallbackTitle="Special Analysis Logbook" />
