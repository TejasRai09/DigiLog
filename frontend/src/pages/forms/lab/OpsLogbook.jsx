...
import { buildOpsLogbookReview } from '../../../config/gsmaFormReviewBuilders';
...
const OpsLogbook = () => {
  ...
  const { reviewOpen, submitting, openReview, closeReview, confirmSubmit } = useGsmaFormReview({
    ...
    submit: async () => {
      await gsmaSubmitRequest(
        () => api.post('/forms/ops_logbook', form),
        'Operations Logbook submitted!',
      );
      setForm(INITIAL);
    },
  });
  ...
  const reviewConfig = useMemo(
    () => (reviewOpen ? buildOpsLogbookReview(form) : null),
    [reviewOpen, form],
  );

  return (
    <main className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-8">
      <FormPageHeader formKey="ops_logbook" fallbackTitle="Operations Logbook" />
