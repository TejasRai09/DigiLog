...
def is_schedule_serial_header(text: str) -> bool:
    """Accept Sr.No. / Sr. No. / SrNo / S.No / SN variants from plant workbooks."""
    h = norm(text)
    compact = re.sub(r"[\s.]+", "", h)
    # Compact forms: SRNO, SNO, SN (and prefixes like SRNO1)
    if compact in {"SN", "SRNO", "SNO"} or compact.startswith("SRNO") or compact.startswith("SNO"):
        return True
    h2 = h.rstrip(".")
    return (
        h2 in {"SN", "SR.NO", "SR.NO.", "S.NO.", "S.NO", "S NO", "SR NO", "SR. NO", "SRNO"}
        or h2.startswith("SR.NO")
        or h2.startswith("S.NO")
        or h2.startswith("SR. NO")
    )
