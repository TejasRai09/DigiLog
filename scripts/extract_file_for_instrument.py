#!/usr/bin/env python3
"""
Extract normalized equipment history from File for Instrument.xlsx.

Output: instrument-equipment-history.xlsx
  - Sheet Map
  - EQUIPMENT LIFE HISTORY CARD
  - EQUIPMENT SPECIFICATION (section / sub_section when present)
  - MAINTENANCE SCHEDULE (dynamic interval columns)
...
SOURCE = ROOT / "File for Instrument.xlsx"
OUTPUT = ROOT / "instrument-equipment-history.xlsx"

SKIP_SHEETS = {"index", "sheet1"}
...
def should_skip_sheet(name: str) -> bool:
    return norm(name.strip()) in {norm(s) for s in SKIP_SHEETS}
...
INSTRUMENT_SCHEDULE_INTERVALS = tuple(SCHEDULE_INTERVALS)
