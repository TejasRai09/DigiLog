#!/usr/bin/env python3
"""
Extract Raw Mill House / Raw House electrical life history.

Source (hierarchy in first sheet, cards in remaining sheets):
  07_Final Raw-mill_house-electrical-11082026.xlsx

Output (DigiLog/backend/backlog-data/mill data/):
  mill-house-electrical-equipment-history-11082026.xlsx
  mill-house-electrical-extract-audit-11082026.xlsx
"""
...
SOURCE_FILE = ROOT / "07_Final Raw-mill_house-electrical-11082026.xlsx"
OUTPUT = OUTPUT_DIR / "mill-house-electrical-equipment-history-11082026.xlsx"
AUDIT_OUTPUT = OUTPUT_DIR / "mill-house-electrical-extract-audit-11082026.xlsx"

SKIP_SHEETS = {"index", "summary index", "summary link", "sheet1", "raw electrical list"}
...
def load_hierarchy_rows(wb) -> list[dict]:
    ...
    for r in range(1, min(12, ws.max_row) + 1):
        ...
        if any("tag" in c for c in cells) and any("section" in c or "equipment" in c for c in cells):
            header_row = r
            break
    rows: list[dict] = []
...
def write_audit(hierarchy_rows: list[dict], matched_sheets: dict[str, list[str]], extra_sheets: list[list[str]]) -> Path:
    rows = []
    ...
    write_data_sheet(
        ...
        [
            ...
            ["Data sheets extracted not in hierarchy", len(extra_sheets)],
        ],
        {"A": 48, "B": 12},
    )
    ws = write_data_sheet(
...
def main() -> None:
    ...
    for sheet_name in src.sheetnames[1:]:
        ...
        if meta is None:
            extra_sheets.append(
                [source_name, sheet_name, equip_tag or "(blank)", fields.get("NAME OF EQUIPMENT", "")]
            )
        ...
        out_tag = meta["raw_tag"] if meta else equip_tag
        ...
        all_life.append(
            [
                ...
                sheet_name,
                out_tag,
                fields["NAME OF EQUIPMENT"] or (meta or {}).get("sub_equipment", ""),
                fields["LOCATION"] or (meta or {}).get("hist_location", ""),
                fields["DATE OF COMMISSIONING"],
                (meta or {}).get("main_equipment", ""),
                (meta or {}).get("sub_equipment", ""),
                (meta or {}).get("department", "ELECTRICAL"),
            ]
        )
        all_spec.extend(
            prepend_source_file(
                insert_tag_column(extract_specification_rows(ws, sheet_id, sheet_name), out_tag),
                source_name,
            )
        )
        all_schedule.extend(
            prepend_source_file(
                insert_tag_column(extract_maintenance_schedule(ws, sheet_id, sheet_name), out_tag),
                source_name,
            )
        )
        all_history.extend(
            prepend_source_file(
                insert_tag_column(extract_maintenance_history(ws, sheet_id, sheet_name), out_tag),
                source_name,
            )
        )
    ...
    missing = [m for m in hierarchy_rows if m["raw_tag"] and "/" in m["raw_tag"] and norm_tag(m["raw_tag"]) not in matched_sheets]
    print(f"Extracted sheets: {len(all_mappings)}")
    print(f"Hierarchy tags missing in data: {len(missing)}")
    for m in missing:
        print(f"  - {m['raw_tag']}  ({m['sub_equipment']})")
    print(f"Data sheets whose tag is not on hierarchy sheet: {len(extra_sheets)}")
    ...
    write_data_sheet(out, "EQUIPMENT LIFE HISTORY CARD", LIFE_COLUMNS, all_life, {
        "A": 48, "B": 16, "C": 32, "D": 42, "E": 40, "F": 28, "G": 20, "H": 28, "I": 36, "J": 14,
    })
    write_data_sheet(out, "EQUIPMENT SPECIFICATION", SPEC_COLUMNS, all_spec, {
        "A": 48, "B": 16, "C": 32, "D": 42, "E": 14, "F": 18, "G": 28, "H": 36,
    })
    write_data_sheet(out, "MAINTENANCE SCHEDULE", SCHEDULE_COLUMNS, all_schedule, {
        "A": 48, "B": 16, "C": 32, "D": 42, "E": 8, "F": 22, "G": 42,
        "H": 8, "I": 8, "J": 8, "K": 10, "L": 12, "M": 8, "N": 10, "O": 10, "P": 10, "Q": 16,
    })
    write_data_sheet(out, "EQUIPMENT MAINTENANCE HISTORY", HISTORY_COLUMNS, all_history, {
        "A": 48, "B": 16, "C": 32, "D": 42, "E": 18, "F": 14, "G": 16,
