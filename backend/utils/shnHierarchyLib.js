...
async function fetchAllActiveNodes(conn = pool) {
  const [rows] = await conn.execute(
    `SELECT ${NODE_SELECT} FROM shn_hierarchy_node WHERE is_active = 1 ORDER BY sort_order ASC, id ASC`,
  );
...
/** Section → category, Location → subcategory (same pattern as power plant). */
...
async function getNodeById(dbId) {
  const [[row]] = await pool.execute(
    `SELECT ${NODE_SELECT} FROM shn_hierarchy_node WHERE id = ? AND is_active = 1`,
    [dbId],
  );
  return row ? rowToPayload(row) : null;
}

async function resolveShnEquipIdForNode(node) {
  if (node.shnEquipId) return node.shnEquipId;
...
async function findSiblingNameConflict(parentId, name, excludeId = null) {
  ...
  let sql = `SELECT id, name FROM shn_hierarchy_node
