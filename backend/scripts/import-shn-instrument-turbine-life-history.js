/**
 * Import Sugar House Equipment History (yellow-highlighted Instrument/Turbine
 * rows) from the normalized migration workbook into shn_* tables and link
 * hierarchy leaves.
 *
 * Unlike the plain electrical/mechanical/instrument importers, this source
 * mixes disciplines row-by-row (46 instrument field devices + 5 steam
 * turbines that are mechanical equipment). The workbook's "Department"
 * column on each EQUIPMENT LIFE HISTORY CARD row says which one it is, so
 * every card's specs / schedule / history rows are tagged with that card's
 * own discipline instead of one hardcoded section.
 *
 * Rules:
 *   - Match hierarchy leaves by equip_no (tag); if several leaves share the
 *     same tag, also match Sub Equipment name (do not fan out to siblings)
 *   - Do NOT overwrite equipment name — use hierarchy lookup_name
 *   - Specs / schedule / history: section = card's Department
 *     (instrument | mechanical | electrical | civil, default instrument);
 *     sub_section = Excel "NAME OF EQUIPMENT" (fallback: Others)
 *
 * Usage (from backend/):
 *   npm run db:import-shn-instrument-turbine-260826 -- --dry-run
 *   npm run db:import-shn-instrument-turbine-260826 -- --replace
 */
...
const DEFAULT_SECTION = 'instrument';
...
/** Card-level discipline, derived from the workbook's own Department column. */
function resolveSection(departmentValue) {
  const d = String(departmentValue || '').trim().toUpperCase();
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i += 1) {
    ...
    if (a === '--dry-run') opts.dryRun = true;
      ...
      console.log(`Usage: node scripts/import-shn-instrument-turbine-life-history.js [options]

Options:
  --dry-run          Parse and match only; no DB writes
  --replace          Replace existing shn_equipment (by tag/equip_no) then re-import
  --file <path>      Source workbook (default: backlog-data/mill data/migration files-24-08-26/yellow-instrument-turbine-equipment-history-260826.xlsx)
...
function parseScheduleSheetRows(rawRows, section, subSection = SUB_SECTION) {
  const parsed = rawRows
    .map((s, index) => {
      ...
      return {
        ...
        equipment_refs: [{ section, sub_section: subSection }],
        no,
        comp,
        act,
        iv_W,
...
function buildSubsectionsMeta(section, equipmentNames = []) {
  const names = [...new Set(
    equipmentNames
      .map((n) => trim(n))
      .filter(Boolean),
  )];
  const bucket = names.length ? names : [SUB_SECTION];
  return JSON.stringify({
    mechanical: section === 'mechanical' ? bucket : [],
    civil: section === 'civil' ? bucket : [],
    instrument: section === 'instrument' ? bucket : [],
    electrical: section === 'electrical' ? bucket : [],
  });
}
...
function loadWorkbook(filePath) {
  ...
  for (const row of lifeRows) {
    ...
    const section = resolveSection(row.Department);
    ...
    specs.push({
      ...
      val: buildSubsectionsMeta(section, equipmentNames),
    });

    const schedule = parseScheduleSheetRows(scheduleById.get(sheetId) || [], section, primarySubSection);
    ...
    const history = (historyById.get(sheetId) || [])
      .map((h) => {
        ...
        return {
          ...
          equipment_refs: [{ section, sub_section: primarySubSection }],
...
async function main() {
  ...
  console.log(`Discipline breakdown (per card, department: ${DEPT}):`, bySection);
  ...
  try {
    ...
    for (let i = 0; i < cards.length; i += 1) {
      ...
      if (opts.dryRun) {
        ...
        console.log(
          `[dry-run] ${card.tag} (${card.section}) / ${equipmentName} -> leaf=${leaf ? leaf.id : 'NONE'} `
      ...
      try {
        ...
        console.log(`[ok] ${card.tag} (${card.section}) -> shn_equipment#${equipId}${leaf ? ` linked to leaf#${leaf.id}` : ' (no hierarchy leaf)'}`);
      } catch (err) {
        await conn.rollback();
        stats.failed += 1;
