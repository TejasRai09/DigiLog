/**
 * Import DS House mechanical life history for the 18-09-26 batch
 * (ZIL/GSM/DS/117–182) into shn_* tables.
 *
 * Match leaves by tag; if several leaves share a tag, also match Sub Equipment.
 * Do not copy one card onto every sibling.
 *
 * Usage (from backend/):
 *   npm run db:import-shn-ds-house-mechanical-180926:history -- --dry-run
 *   npm run db:import-shn-ds-house-mechanical-180926:history -- --replace
 */
require('../config/env');
...
const DEFAULT_FILE = path.join(
  BACKLOG_DIR,
  'mill data/migration files-18-09-26/ds-house-mechanical-equipment-history-180926.xlsx',
);
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i += 1) {
    ...
    if (a === '--dry-run') opts.dryRun = true;
      ...
      console.log(`Usage: node scripts/import-shn-ds-house-mechanical-life-history-180926.js [options]

Options:
  --dry-run          Parse and match only; no DB writes
  --replace          Replace matching cards; clear unmatched siblings of those tags
  --file <path>      Source workbook (default: ds-house-mechanical-equipment-history-180926.xlsx)
...
function equipmentNamesMatch(a, b) {
  ...
  if (equipmentNameKeyVariants(b).some((key) => left.has(key))) return true;
