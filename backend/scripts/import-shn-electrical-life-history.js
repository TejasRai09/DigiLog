/**
 * Import Sugar House Equipment History (electrical) from the normalized
 * mill-house electrical life-history workbook into shn_* tables and link
 * hierarchy leaves.
 *
 * Rules:
 ...
 *
 * Usage (from backend/):
 *   npm run db:import-shn-electrical-life-history -- --dry-run
 *   npm run db:import-shn-electrical-life-history -- --replace
 *   npm run db:import-shn-electrical-life-history -- --replace --file "backlog-data/mill data/mill-house-electrical-equipment-history.xlsx"
 */
...
const DEFAULT_FILE = path.join(
  BACKLOG_MILL_DIR,
  'mill-house-electrical-equipment-history.xlsx',
);
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i += 1) {
    ...
    if (a === '--dry-run') opts.dryRun = true;
      ...
      console.log(`Usage: node scripts/import-shn-electrical-life-history.js [options]

Options:
  --dry-run          Parse and match only; no DB writes
  --replace          Replace existing shn_equipment (by tag/equip_no) then re-import
  --file <path>      Source workbook (default: mill-house-electrical-equipment-history.xlsx)
...
function buildSubsectionsMeta(equipmentNames = []) {
  ...
  return JSON.stringify({
    ...
    civil: [],
    instrument: [],
    electrical: names.length ? names : [SUB_SECTION],
  });
}
