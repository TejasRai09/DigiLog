/**
 * Import DS House mechanical hierarchy for the 18-09-26 batch
 * (ZIL/GSM/DS/117–182) into shn_hierarchy_node.
 *
 * Default file: ds-house-mechanical-hierarchy-180926.xlsx
 * APPENDS under existing Sugar Plant root. Duplicate tags stay as separate
 * leaves (tag + Sub Equipment).
 *
 * Usage (from backend/):
 *   npm run db:import-shn-ds-house-mechanical-180926:hierarchy -- --dry-run
 *   npm run db:import-shn-ds-house-mechanical-180926:hierarchy
 */
require('../config/env');
...
const DEFAULT_XLSX_LIST = [
  path.join(__dirname, '../backlog-data/mill data/migration files-18-09-26/ds-house-mechanical-hierarchy-180926.xlsx'),
  path.join(
    __dirname,
    '../../../Extraction_files-sugar/new files - 180926/ds-house-mechanical-hierarchy-180926.xlsx',
  ),
];
...
function departmentColumn(row) {
  return trim(row.Department || row.department);
}
...
function isMechanicalDept(dept) {
  const upper = String(dept || '').toUpperCase().replace(/\s+/g, '');
  return upper.includes('MECHANICAL') || upper.includes('MECHNICAL') || upper.startsWith('MECH');
}

function pickMechanicalTag(tags) {
  if (!tags.length) return '';
  if (tags.length === 1) return tags[0];
  const gsm = tags.find((t) => compactTag(t).startsWith('ZIL/GSM/'));
  if (gsm) return gsm;
  const nonSug = tags.find((t) => !compactTag(t).startsWith('ZIL/SUG'));
  if (nonSug) return nonSug;
...
function parseArgs(argv) {
  ...
  if (!args.files.length) {
    const found = DEFAULT_XLSX_LIST.find((f) => require('fs').existsSync(f));
    args.files = found ? [found] : [];
  }
  return args;
}
...
function loadRows(xlsxPath) {
  ...
  for (let r = headerRow + 1; r < aoa.length; r += 1) {
    ...
    headers.forEach((h, i) => {
      if (!h) return;
      const val = cells[i] ?? '';
      // Duplicate header names (e.g. multiple "Department" cols) — keep first non-empty.
      if (!(h in row) || !trim(row[h])) {
        row[h] = val;
      }
    });
    sheetRows.push(row);
  ...
  for (const row of sheetRows) {
    ...
    const rawTagCell = tagNoColumn(row);
    const equipNo = pickMechanicalTag(splitTags(rawTagCell));
