/**
 * Extract Production House centrifugal / drier house specs + maintenance actions
 * from DS Centrifugal Drier House History Record_3.xlsx
 *
 * Usage (from backend/):
...
const SRC_CANDIDATES = [
  path.join('C:', 'vivek', 'PLANT', 'DS Centrifugal Drier House History Record_3.xlsx'),
  path.join('C:', 'vivek', 'PLANT', 'production equipments hisotoru files', 'DS Centrifugal Drier House History Record_3.xlsx'),
];
const OUT_DIR = path.join(__dirname, '..', 'backlog-data', 'production-equipments');
const OUT_XLSX = path.join(OUT_DIR, 'ds-centrifugal-drier-equipment-history.xlsx');
const OUT_JSON = path.join(OUT_DIR, 'ds-centrifugal-drier-equipment-history.json');
const OUT_AUDIT = path.join(OUT_DIR, 'ds-centrifugal-drier-extract-audit.xlsx');
...
const SN_RE = /^(?:\d{1,2}|[a-d]|i{1,3}|iv)$/i;
...
function classifyType(sheetName, equipmentName) {
  ...
  if (n.includes('batch')) return 'Batch centrifugal';
  if (n.includes('heater')) return 'Transient heater';
  if (n.includes('conti')) return 'Continuous centrifugal';
  if (n.includes('pugmill')) return 'Pugmill';
  if (n.includes('magma')) return 'Magma mixer';
  if (n.includes('melter')) return 'Melter';
  if (n.includes('hopper')) return 'Hopper';
  if (n.includes('elevator')) return 'Elevator';
  if (n.includes('grader')) return 'Sugar grader';
  if (n.includes('conveyor') || n.includes('belt')) return 'Belt conveyor';
  if (n.includes('dust') || n.includes('dus collector')) return 'Dust collector';
  if (n.includes('spray pond')) return 'Spray pond';
  if (n.includes('injection pump')) return 'Injection pump';
  if (n.includes('spray pump')) return 'Spray pump';
  return 'Equipment';
}
...
function inferSeasonFromAction(action, fallback) {
  ...
  if (/\bin-season\b/.test(t) || /\bduring(\s+the)?\s+season\b/.test(t)) return 'Season';
  return fallback;
}
...
function findRightTableStartCol(ws, headerRow, startCol, maxCol) {
  const last = Math.max(ws.rowCount || 0, headerRow + 4);
  for (let r = 1; r <= Math.min(last, headerRow + 6); r++) {
    for (let c = startCol + 3; c <= maxCol; c++) {
      const t = norm(cellText(ws.getRow(r).getCell(c)));
      if (SN_HEADER_RE.test(t)) return c;
    }
  }
  return null;
...
function extractSheet(ws) {
  ...
  if (rightTableCol) {
    issues.push(`Right-hand table from column ${rightTableCol} ignored (calc / diagram)`);
  } else if (maxCol > startCol + 3) {
    issues.push(`Columns beyond the left spec table ignored`);
  }
  ...
  const pushHistory = (yearHit, actionText, r) => {
    ...
    if (!action) return;
    const actionMatch = action.match(ACTION_RE);
    history.push({
      equipmentName,
      ...
      action: actionMatch ? (norm(actionMatch[2]) || action) : action,
  ...
  for (let r = 1; r <= maxRow; r++) {
    const leftYear = startCol > 1 ? parseYear(norm(cellText(ws.getRow(r).getCell(1)))) : null;
    ...
    if (leftYear) {
      ...
      if (!leftYear.labelled) yearOnlyHistory = true;
      const work = uniqueJoin([c1, c2, c3]);
      if (work) pushHistory(currentYear, work, r);
      continue;
    }
    if (!line) continue;
    ...
    const farText = (() => {
      const from = rightTableCol || startCol + 3;
      const parts = [];
      for (let c = from; c <= Math.min(maxCol, from + 8); c++) {
        parts.push(cellText(ws.getRow(r).getCell(c)));
      }
    ...
    if (mode === 'history') {
      ...
      if (isSectionTitle(c1, c2, c3) || (SN_RE.test(c1) && c2) || looksLikeEquipmentTitle(c1 || c2)) {
        ...
        if (currentYear && (c2 || c3) && !SN_RE.test(c1) && !parseYear(c1)) {
  ...
  if (currentYear && !history.some((h) => h.year === currentYear.year)) {
    history.push({
      equipmentName,
      ...
      actionNo: 1,
      action: '',
      labelledSeason: currentYear.labelled,
      sourceRow: maxRow,
    });
    issues.push(`Year ${currentYear.year} present with no work text`);
  }
...
async function writeAudit(cards, src) {
  ...
  [
    ...
    ['Season labelling', 'Years often sit in column A with work in the spec columns. Recorded as Off-Season.'],
    ['Side tables', 'If a second SN header sits to the right, that calc/diagram table is not extracted. Specs come from the left SN / Particular / Detail table only.'],
  ...
  try {
    ...
    if (e && e.code === 'EBUSY') {
      const alt = path.join(OUT_DIR, 'ds-centrifugal-drier-extract-audit-new.xlsx');
      await wb.xlsx.writeFile(alt);
      console.warn('Audit file was open; wrote', alt);
      return;
    }
    throw e;
  }
