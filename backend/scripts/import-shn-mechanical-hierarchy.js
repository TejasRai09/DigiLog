/**
 * Import Sugar House mechanical equipment hierarchy (Boiling House list) into
 * shn_hierarchy_node.
 *
 ...
 *
 * Rules:
 *   - Only rows where Department contains MECHANICAL
 *   - Dual-tag cells (instrument + mechanical): keep only mechanical (ZIL/GSM/...) tag
 *   - APPENDS under existing Sugar Plant root (does not clear other disciplines)
 *
 * Usage (from backend/):
 *   npm run db:import-shn-mechanical-hierarchy -- --dry-run
 *   npm run db:import-shn-mechanical-hierarchy
 */
require('../config/env');
...
function isMechanicalDept(dept) {
  return String(dept || '').toUpperCase().includes('MECHANICAL');
}
